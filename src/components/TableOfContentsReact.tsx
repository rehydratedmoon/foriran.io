...
const TableOfContents: FunctionalComponent<{ headings: MarkdownHeading[] }> = ({ headings = [] }) => {
  ...
  return (
    <>
      <h2 id={onThisPageID} className='font-family-content c-sand10 text-sm pt-2'>
        فهرست
      </h2>
      <ul ref={toc} className=' list-none p-0 vertical_dashed-line'>
        {headings
          ...
          .map((heading) => (
            <li
              className={`c-sand10 truncate ${heading.depth === 2 ? ' mt-3 mb-1 py-0  pis-4 ' : ''}  ${
                heading.depth === 3 ? 'py-0.4 pis-12 text-sm c-sand11' : ''
              } ${currentID === heading.slug ? 'bg-sand3' : ''}`}
            >
